      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex flex-col md:flex-row gap-4 flex-1">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-sage-green/50 h-4 w-4" />
            <Input
              placeholder="Search servers..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10 bg-charcoal-light/50 border-amber-gold/20 text-sage-green placeholder:text-sage-green/50"
            />
          </div>
          
          <Select value={selectedGameType} onValueChange={setSelectedGameType}>
            <SelectTrigger className="w-full md:w-48 bg-charcoal-light/50 border-amber-gold/20 text-sage-green">
        ...
        <div className="grid gap-4">
          <AnimatePresence>
            {filteredServers.map((server, index) => (
              ...
              >
                <Card className="bg-charcoal-light/80 border-amber-gold/20 hover:border-amber-gold/40 transition-all duration-300 group">
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <div className={`w-3 h-3 rounded-full ${getStatusColor(server.isOnline)}`} />
                          <h3 className="text-xl font-rye text-amber-gold group-hover:text-amber-gold/80 transition-colors">
                            {server.name}
                          </h3>
                        ...
                        <p className="text-sage-green/80 mb-4 line-clamp-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div className="flex items-center gap-2">
                        <Users className="h-4 w-4 text-amber-gold" />
                        <span className="text-sage-green text-sm">
                          {server.playerCount}/{server.maxPlayers}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Clock className={`h-4 w-4 text-amber-gold`} />
                        <span className={`text-sm ${getPingColor(server.ping)}`}>
                          {server.ping}ms
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <ServerIcon className="h-4 w-4 text-amber-gold" />
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-sage-green/60">
