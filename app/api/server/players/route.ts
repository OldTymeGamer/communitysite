...
export async function GET() {
  const serverIp = process.env.GAME_SERVER_IP || "localhost"
  const serverPort = process.env.GAME_SERVER_PORT || "30120"
  ...
  try {
    ...
    try {
      ...
      if (listResponse.ok) {
        ...
        if (listData?.Data) {
          ...
          serverName = listData.Data.hostname || "Game Server"
        }
      }
    } catch (listError) {
    ...
    console.error("Error fetching game server info:", error)
    return NextResponse.json({ 
      count: 0,
      maxPlayers: 32,
